...
public class Delivery : EVObject {
    ...
    public var contactPreferences : String? = nil
    ...
    public func setDeliveryAddress(address: String?,city: String?,state: String?,postalCode: String?,country:String?,longitude:Double?,latitude:Double?,internalNumber:String?,externalNumber:String?,betweenStreet1:String?,betweenStreet2:String?,addressReference:String?,neighborhood:String?) {
    ...
    public func setApplicationInfo(appId:Int?,appName:String?){
        ...
        device.pushToken = UserCurrentSession.sharedInstance().deviceToken
        ...
        if self.consumer?.user?.person == nil {
            self.consumer?.user?.person = Person()
            self.consumer?.user?.person?.contact = []
        }
    ...
    public func addContactInfo(typeContact:ContactMean,value:String,ext:String) {
        
        let contactUser = Contact()
        contactUser.mean = typeContact.rawValue
        contactUser.value = value
        contactUser.ext = ext
        
        if self.consumer?.user?.person?.contact == nil {
            self.consumer?.user?.person?.contact = []
        }
        
        self.consumer?.user?.person?.contact?.append(contactUser)
