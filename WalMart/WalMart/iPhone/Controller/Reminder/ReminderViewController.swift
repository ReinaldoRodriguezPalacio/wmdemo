...
class ReminderViewController: NavigationViewController,CalendarViewDelegate, TPKeyboardAvoidingScrollViewDelegate, UIScrollViewDelegate, AlertPickerSelectOptionDelegate{
    ...
    override func viewWillLayoutSubviews() {
        ...
        if IS_IPAD {
            self.content!.frame = CGRect(x: 0.0, y: 46.0,  width: self.view.bounds.width, height: 300 )
    ...
    func didSelectOptionAtIndex(_ indexPath: IndexPath){
        ...
        self.selectedPeriodicity = (indexPath as NSIndexPath).row
        self.frequencyField?.text = self.reminderService!.options[(indexPath as NSIndexPath).row]
