...
class MyAddressViewController: NavigationViewController,  UITableViewDelegate, UITableViewDataSource , AddressViewCellDelegate, SWTableViewCellDelegate {
    ...
    override func viewWillLayoutSubviews() {
        ...
        let heightEmptyView = Int(bounds.height) - 44
        
        self.table!.frame =  CGRect(x: 0,  y: self.header!.frame.maxY + SELECTORH , width: bounds.width, height: bounds.height - self.header!.frame.maxY - SELECTORH - 44)
        //tamaño
        self.newAddressButton!.frame = CGRect(x: bounds.width - (buttonWidth + 16.0), y: (header!.bounds.height - buttonHeight)/2, width: buttonWidth, height: buttonHeight)

        //self.newAddressButton!.frame = CGRectMake( self.view.bounds.maxX - 165.0, 12.0, 75.0, 22.0 )
        self.titleLabel!.frame = CGRect(x: self.newAddressButton!.frame.width , y: 0, width: bounds.width - (self.newAddressButton!.frame.width * 2), height: self.header!.frame.maxY)
        
        self.emptyView!.frame = CGRect(x: 0, y: 46, width: Int(bounds.width), height: heightEmptyView)
