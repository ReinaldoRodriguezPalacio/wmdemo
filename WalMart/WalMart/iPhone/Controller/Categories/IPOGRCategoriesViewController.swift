...
class IPOGRCategoriesViewController: NavigationViewController, UITableViewDataSource,UITableViewDelegate,IPOGRDepartmentSpecialTableViewCellDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (indexPath as NSIndexPath).row % 2 == 0 || collapsed {
            let cellDept = tableView.dequeueReusableCell(withIdentifier: "celldepartment", for: indexPath) as! GRDepartmentTableViewCell
            var rowforsearch = (indexPath as NSIndexPath).row
            if !collapsed {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !((indexPath as NSIndexPath).row % 2 == 0) && !self.collapsed {
            rowforsearch = (indexPath as NSIndexPath).row - 1
            newIndex = IndexPath(row:  (indexPath as NSIndexPath).row - 1, section: (indexPath as NSIndexPath).section)
        }
    ...
    func changeStore(){
        if titleLabel!.text! == NSLocalizedString("profile.default.store", comment: "") && UserCurrentSession.sharedInstance.addressId == nil {
            let noAddressView = AddressNoStoreView(frame: CGRect(x: 0,y: 0,width: 288,height: 210))
