...
class NewListTableViewCell : UITableViewCell, UITextFieldDelegate {
    ...
    override func layoutSubviews() {
        ...
        self.separatorView!.frame = CGRect(x: 0.0, y: bounds.height - 1.0, width: self.contentView.frame.size.width, height: 1.0)
    }
    ...
    class func isValidName(_ field:UITextField?,showAlert:Bool) -> Bool {
        if field == nil {
            return false
        }
        ...
        let cleanedName = (trimmedString.components(separatedBy: alphanumericset)).componentsJoined(by: "")
