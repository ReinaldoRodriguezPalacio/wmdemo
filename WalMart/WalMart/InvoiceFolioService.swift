//
//  InvoiceFolioService.swift
//  WalMart
//
//  Created by Vantis on 05/05/17.
//  Copyright © 2017 BCG Inc. All rights reserved.
//

import Foundation

class InvoiceFolioService : BaseService {
    
    func callService(params: [String:Any], successBlock:(([String:Any]) -> Void)?, errorBlock:((NSError) -> Void)? ) {
        let parameters : [String:Any] = ["findFolioInvoiceByTicketRequest" : params]
        self.setManagerTempHeader()
        self.callPOSTService(parameters,
                             successBlock: { (resultCall:[String:Any]) -> Void in
                                let values = resultCall["findFolioInvoiceByTicketResponse"] as! [String:Any]
