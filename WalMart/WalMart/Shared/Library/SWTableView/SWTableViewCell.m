...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if([keyPath isEqualToString:kTableViewPanState] && object == _tableViewPanGestureRecognizer)
    {
        ...
        } else if(_tableViewPanGestureRecognizer.state == UIGestureRecognizerStateFailed) {
            //[self hideUtilityButtonsAnimated:YES];
            //[self showLeftUtilityButtonsAnimated:true];
        }
    }
}
...
- (CGPoint)contentOffsetForCellState:(SWCellState)state
{
    ...
    //NSLog(@"ScrollOffset %f",scrollPt.x);
    return scrollPt;
}
...
- (void)updateCellState
{
    ...
    self.tapGestureRecognizer.enabled = !self.isEditing;
}
