...
- (BOOL)TPKeyboardAvoiding_focusNextTextField {
    ...
    UIView* view =  [self TPKeyboardAvoiding_focusNextTextFieldAfterView:firstResponder];
    if ( view ) {
...
-(UIView*)TPKeyboardAvoiding_focusNextTextFieldAfterView:(UIView*)lastFirstResponder {
   CGFloat minY = CGFLOAT_MAX;
    UIView *view = nil;
    [self TPKeyboardAvoiding_findTextFieldAfterTextField:lastFirstResponder beneathView:self minY:&minY foundView:&view];
    return view;
}
    ...
    for ( UIView *childView in view.subviews ) {
        ...
        if ( ([childView isKindOfClass:[UITextField class]] || [childView isKindOfClass:[UITextView class]]) ) {
            ...
            if ( childView != priorTextField && CGRectGetMinY(frame) >= priorFieldOffset && CGRectGetMinY(frame) < *minY) {
                if (childView.frame.origin.y == priorTextField.frame.origin.y && priorTextField.frame.origin.x >=  childView.frame.origin.x ) continue;
                if ( !((UITextField*)childView).enabled )continue;
