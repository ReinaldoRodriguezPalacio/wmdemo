...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    if ([view conformsToProtocol:@protocol(iCarouselItem)] && [view respondsToSelector:@selector(updateViewForOffset:)]) {
        UIView <iCarouselItem> * item = (UIView <iCarouselItem> *) view;
        [item updateViewForOffset:offset];
    }
...
- (void)step
{
    ...
    if (_toggle != 0.0f)
    {
        NSTimeInterval toggleDuration = _startVelocity? fmin(1.0, fmax(0.0, 1.0 / fabs(_startVelocity))): 1.0;
