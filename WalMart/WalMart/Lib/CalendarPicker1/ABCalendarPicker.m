...
//SAMS
@synthesize tappedOccured = _tappedOccured;
@synthesize headerHeight = _headerHeight;
@synthesize columnTitleHeight = _columnTitleHeight;
...
- (void)leftButtonClicked:(id)sender
{
    ...
    if ([(id)self.delegate respondsToSelector:@selector(calendarPicker:willAnimateWith:)]) {
        [self.delegate calendarPicker:self willAnimateWith:animation];
    }
//    self.highlightedDate = [self.currentProvider dateForPrevAnimation];
    _highlightedDate = [self.currentProvider dateForPrevAnimation];
...
//    self.highlightedDate = [self.currentProvider dateForNextAnimation];
    _highlightedDate = [self.currentProvider dateForNextAnimation];
    [self changeStateTo:self.currentState fromState:self.currentState animation:animation canDiffuse:canDiffuse];
}
...
//    self.highlightedDate = [self.currentProvider dateForLongPrevAnimation];
    _highlightedDate = [self.currentProvider dateForLongPrevAnimation];
...
//    self.highlightedDate = [self.currentProvider dateForLongNextAnimation];
    _highlightedDate = [self.currentProvider dateForLongNextAnimation];
    [self changeStateTo:self.currentState fromState:self.currentState animation:animation canDiffuse:0];
}
    ...
    for (int i = 0 ; i < [self.controls count]; i++)
    {
        ...
        for (int j = 0; j < arr.count; j++)
        {
            ...
            if (CGRectContainsPoint(control.frame, point))
            {
                ...
                {
                    self.tappedOccured = NO; //Solo para identificar que es por animacion
...
- (void)updateColumnNamesForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider
{
    ...
    for (int j = 0; j < columnsCount; j++)
    {
        ...
        UILabel * columnLabel = [[UILabel alloc] initWithFrame:CGRectMake(floor(j*buttonWidth),self.headerHeight,buttonWidth,self.columnTitleHeight)];
...
- (void)updateTitleForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider
{
    ...
    self.titleLabel.frame = CGRectMake(self.mainTileView.center.x - titleWidth/2, 0, titleWidth, self.headerHeight);
    self.titleButton.frame = CGRectMake(self.titleLabel.center.x - buttonWidth/2, 0, buttonWidth, self.headerHeight);
...
- (void)updateArrowsForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider
{
    ...
    self.leftArrow.frame = CGRectMake((self.longLeftArrow.hidden ? 0 : 35), 0, 40, self.headerHeight);
    ...
    self.rightArrow.frame = CGRectMake(self.bounds.size.width-40-(self.longRightArrow.hidden ? 0 : 35), 0, 40, self.headerHeight);
    ...
    {
        self.longLeftArrow.frame = CGRectMake(0, 0, 35, self.headerHeight);
    ...
    {
        self.longRightArrow.frame = CGRectMake(self.bounds.size.width-35, 0, 35, self.headerHeight);
...
{
    if ([(UIGestureRecognizer*)[self.leftArrow.gestureRecognizers lastObject] state] == UIGestureRecognizerStateBegan
        || [(UIGestureRecognizer*)[self.rightArrow.gestureRecognizers lastObject] state] == UIGestureRecognizerStateBegan
        || [(UIGestureRecognizer*)[self.longLeftArrow.gestureRecognizers lastObject] state] == UIGestureRecognizerStateBegan
        || [(UIGestureRecognizer*)[self.longRightArrow.gestureRecognizers lastObject] state] == UIGestureRecognizerStateBegan)
    {
        return;
    }
...
- (void)updateButtonsForProvider:(id<ABCalendarPickerDateProviderProtocol>)provider andState:(ABCalendarPickerState)state
{
    ...
    CGFloat buttonHeight = columnsCount > 1 ? buttonWidth : 50.0f;
...
//            UIControl * control = [self.styleProvider calendarPicker:self cellViewForTitle:label andState:self.currentState];
            UIControl * control = [self.styleProvider calendarPicker:self cellViewForTitle:label andState:state];
           ...
           canDiffuse:(NSInteger)canDiffuse
{
    ...
    if ([(id)self.delegate respondsToSelector:@selector(calendarPickerHeightForHeader:)]) {
        self.headerHeight = [self.delegate calendarPickerHeightForHeader:self];
    }
    
    if ([(id)self.delegate respondsToSelector:@selector(calendarPickerHeightForColumnHeader:)]) {
        self.columnTitleHeight = [self.delegate calendarPickerHeightForColumnHeader:self];
    }
    ...
    CGFloat newFrameHeight = (self.headerHeight + self.columnTitleHeight) + buttonHeight*rowsCount + 1;
    ...
    {
        self.mainTileView = [[UIView alloc] initWithFrame:CGRectMake(0,(self.headerHeight + self.columnTitleHeight),self.frame.size.width,self.frame.size.height - (self.headerHeight + self.columnTitleHeight))];
        ...
        self.mainTileView.backgroundColor = [UIColor whiteColor]; //SAMS
    ...
    if (animation == ABCalendarPickerAnimationNone)
    {
        ...
        self.frame = CGRectMake(self.frame.origin.x,
                                ...
                                newFrameHeight);
        self.mainTileView.frame = CGRectMake(0, (self.headerHeight + self.columnTitleHeight) ,self.frame.size.width,self.frame.size.height - (self.headerHeight + self.columnTitleHeight));
             ...
             weekdaysProvider:(id<ABCalendarPickerDateProviderProtocol>)weekdaysProvider
               ...
               monthsProvider:(id<ABCalendarPickerDateProviderProtocol>)monthsProvider
                yearsProvider:(id<ABCalendarPickerDateProviderProtocol>)yearsProvider
                 ...
                 initialState:(ABCalendarPickerState)state
{
    ...
    _highlightedDate = self.selectedDate;
...
- (void)initWithDefaultProvidersWhitState:(ABCalendarPickerState)state
{
    [self initWithStyleProvider:[[CustomCalendarStyleProvider alloc] init]
               weekdaysProvider:[[ABCalendarPickerDefaultTripleWeekdaysProvider alloc] init]
                   daysProvider:[[CustomCalendarPickerDefaultDaysProvider alloc] init]
                 monthsProvider:[[ABCalendarPickerDefaultSeasonedMonthsProvider alloc] init]
                  yearsProvider:[[ABCalendarPickerDefaultYearsProvider alloc] init]
                   erasProvider:nil
                   initialState:state];//[[ABCalendarPickerDefaultErasProvider alloc] init]];
}

- (id)init
{
    if (self = [super init])
        [self initWithDefaultProvidersWhitState:ABCalendarPickerStateDays];
    return self;
}
...
- (id)initWithFrame:(CGRect)frame andState:(ABCalendarPickerState)state
{
    if (self = [super initWithFrame:frame])
        [self initWithDefaultProvidersWhitState:state];
    return self;
}

- (id)initWithFrame:(CGRect)frame andState:(ABCalendarPickerState)state andDelegate:(id<ABCalendarPickerDelegateProtocol>) delegate andDataSource:(id<ABCalendarPickerDataSourceProtocol>) dataSource {
