       ...
       print("Articulo seleccionado \(indexPath.row)")
        ...
        if cell!.isKind(of: SearchProductCollectionViewCell.self){
            if indexPath.row < self.allProducts!.count {
                ...
                paginatedProductDetail.detailOf = (self.textToSearch != nil && self.textToSearch != "") ? "Search Results" : (self.eventCode != nil ? self.eventCode! : self.titleHeader!)
            ...
            selectQuantity!.addToCartAction =
                { (quantity:String) in
                    ...
                    if maxProducts >= Int(quantity) {
                        ...
                        let params = self.buildParamsUpdateShoppingCart(cell, quantity: quantity, position: cell.positionSelected, orderByPiece: true, pieces: pieces,commens: "")
